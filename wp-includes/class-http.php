...
class WP_Http {
	...
	function request( $url, $args = array() ) {
		...
		$r['local'] = $homeURL['host'] == $arrURL['host'] || '***REMOVED***' == $arrURL['host'];
		unset( $homeURL );
	 ...
	 *
	 * There are currently issues with "***REMOVED***" not resolving correctly with DNS. This may cause
	...
	function block_request($uri) {
		...
		if ( $check['host'] == '***REMOVED***' || $check['host'] == $home['host'] )
...
class WP_Http_Fsockopen {
	...
	function request($url, $args = array()) {
		...
		//fsockopen has issues with '***REMOVED***' with IPv6 with certain versions of PHP, It attempts to connect to ::1,
		// which fails when the server is not set up for it. For compatibility, always connect to the IPv4 address.
		if ( '***REMOVED***' == strtolower($fsockopen_host) )
