	...
	public function get_base_cost() {
		...
		if ( $this->has_persons() && $this->has_person_types() ) {
			...
			foreach ( $persons as $person ) {
				if ( is_null( $cheapest ) || $person->cost < $cheapest ) {
					$cheapest = $person->cost;
				}
			}
			$base += $cheapest;
		}
	...
	public function get_availability_rules( $for_resource = 0 ) {
		if ( empty( $this->availability_rules[ $for_resource ] ) ) {
			...
			if ( $for_resource ) {
				...
				$rules          = array_merge( $resource_rules, $this->wc_booking_availability, get_option( 'wc_global_booking_availability', array() ) );
			} else {
			...
			foreach ( $rules as $fields ) {
				...
				if ( strrpos( $fields['type'], 'time:' ) === 0 ) {
					...
					if ( $fields['bookable'] === 'yes' ) {
						$this->availability_rules[ $for_resource ][] = array( 'days', $this->get_days_range( $day, $day, true ) );
					}
				} elseif ( strrpos( $fields['type'], 'time' ) === 0 ) {
					if ( $fields['bookable'] === 'yes' ) {
						$this->availability_rules[ $for_resource ][] = array( 'days', $this->get_days_range( 0, 7, true ) );
					}
				}

				if ( $type_availability ) {
					$this->availability_rules[ $for_resource ][] = array( $fields['type'], $type_availability );
				}
			}

			// Reverse order so lower rules are evaluated first
			$this->availability_rules[ $for_resource ] = array_reverse( $this->availability_rules[ $for_resource ] );
		}
