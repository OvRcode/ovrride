...
class WC_Booking_Form {
	...
	public function get_posted_data( $posted = array() ) {
		...
		if ( ! empty( $posted['wc_bookings_field_start_date_year'] ) && ! empty( $posted['wc_bookings_field_start_date_month'] ) && ! empty( $posted['wc_bookings_field_start_date_day'] ) ) {
			$data['_year']  = max( date('Y'), absint( $posted['wc_bookings_field_start_date_year'] ) );
			...
			switch ( $type ) {
				...
				case 'months' :
					...
					while ( $check_date < $data['_end_date'] ) {
						...
						if ( isset( $rules[ $month ] ) ) {
							$fixed_booking_cost = $this->apply_cost( $fixed_booking_cost, $rules[ $month ]['block'][0], $rules[ $month ]['block'][1] );
				...
				case 'weeks' :
					...
					while ( $check_date < $data['_end_date'] ) {
						...
						if ( isset( $rules[ $week ] ) ) {
							$fixed_booking_cost = $this->apply_cost( $fixed_booking_cost, $rules[ $week ]['block'][0], $rules[ $week ]['block'][1] );
				...
				case 'days' :
					...
					while ( $check_date < $data['_end_date'] ) {
						...
						if ( isset( $rules[ $day_of_week ] ) ) {
							$fixed_booking_cost = $this->apply_cost( $fixed_booking_cost, $rules[ $day_of_week ]['block'][0], $rules[ $day_of_week ]['block'][1] );
				...
				case 'custom' :
					...
					while ( $check_date < $data['_end_date'] ) {
						...
						if ( isset( $rules[ $year ][ $month ][ $day ] ) ) {
							$fixed_booking_cost = $this->apply_cost( $fixed_booking_cost, $rules[ $year ][ $month ][ $day ]['block'][0], $rules[ $year ][ $month ][ $day ]['block'][1] );
