	...
	public function save_product_data( $post_id ) {
		...
		for ( $i = 0; $i < $row_size; $i ++ ) {
			...
			switch ( $availability[ $i ]['type'] ) {
				...
				case 'time:6' :
				case 'time:7' :
					$availability[ $i ]['from'] = wc_clean( $_POST[ "wc_booking_availability_from_time" ][ $i ] );
					$availability[ $i ]['to']   = wc_clean( $_POST[ "wc_booking_availability_to_time" ][ $i ] );
				break;
			}
		}
		...
		for ( $i = 0; $i < $row_size; $i ++ ) {
			...
			switch ( $pricing[ $i ]['type'] ) {
				...
				break;
				case 'time' :
					$pricing[ $i ]['from'] = wc_clean( $_POST[ "wc_booking_pricing_from_time" ][ $i ] );
					$pricing[ $i ]['to']   = wc_clean( $_POST[ "wc_booking_pricing_to_time" ][ $i ] );
				break;
				default :
		...
		update_post_meta( $post_id, '_price', get_post_meta( $post_id, '_wc_booking_cost', true ) + get_post_meta( $post_id, '_wc_base_cost', true ) );
	}
