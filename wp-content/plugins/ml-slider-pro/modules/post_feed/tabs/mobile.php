...
<div class="row flex-setting" style="<?php echo esc_attr($hide_css); ?>">
    <label style="margin-right: 4px;"><?php esc_html_e("Hide slide on:", "ml-slider"); ?></label>
    <?php
        $checked_slide = '';
...
<div class="row">
    ...
    <?php
        $checked_caption = '';
        foreach ($screen as $key => $value) {
            $hidecaption = get_post_meta($this->slide->ID, 'ml-slider_hide_caption_' . $value, true);
    ...
    ?>
            <span class="mobile-checkbox-wrap">
                <input type="checkbox" name="attachment[<?php echo esc_attr($this->slide->ID); ?>][hide_caption_<?php echo esc_attr($value); ?>]" class="mobile-checkbox tipsy-tooltip-top" title="<?php echo esc_attr($tooltip); ?>" <?php echo esc_attr($checked_caption); ?> />
                <span class="dashicons <?php echo esc_attr( 'dashicons-' . $value ); ?> "></span>
            </span>
    <?php } ?>
</div>
