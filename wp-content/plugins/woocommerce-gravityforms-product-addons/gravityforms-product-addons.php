		...
		function woocommerce_gravityform_enqueue_scripts() {
			...
			if (is_product()) {
				...
				if ($gravity_form_data && is_array($gravity_form_data)) {
					...
					wp_enqueue_script('wc-gravityforms-product-addons', woocommerce_gravityforms::plugin_url() . '/assets/js/gravityforms-product-addons.js', array('jquery', 'gforms_gravityforms'), true);
				}
			}
		}
						...
						foreach ($form_meta['fields'] as $field) {
							...
							if (!empty($value) && !empty($arr_var)) {
								try {
									...
									$display_value = apply_filters("woocommerce_gforms_order_meta_value", html_entity_decode($display_value), $field, $lead, $form_meta, $item_id, $cart_item);
