...
if ( ! class_exists( 'WC_SC_Product_Fields' ) ) {
	...
	class WC_SC_Product_Fields {
		...
		public function woocommerce_product_options_coupons() {
			...
			?>
			<p class="form-field" id="sc-field"><label for="_coupon_title"><?php _e( 'Coupons', WC_SC_TEXT_DOMAIN ); ?></label> <?php 

			    if ( $this->is_wc_gte_30() ) { ?> 
                
                    <select class="wc-coupon-search" style="width: 50%;" multiple="multiple" id="_coupon_title" name="_coupon_title[]" data-placeholder="<?php echo __( 'Search for a coupon&hellip;', WC_SC_TEXT_DOMAIN ); ?>" data-action="sc_json_search_coupons" data-security="<?php echo wp_create_nonce( 'search-coupons' ); ?>" > <?php
                        ...
                        if ( ! empty( $coupon_titles ) ) {
							...
							foreach ( $coupon_titles as $coupon_title ) {
                                ...
                                if ( ! empty( $discount_type ) ) { 
                                	$discount_type = sprintf( __( ' ( %1$s: %2$s )', WC_SC_TEXT_DOMAIN ), __( 'Type', WC_SC_TEXT_DOMAIN ), $all_discount_types[ $discount_type ] );
								}
					...
					<input type="hidden" class="wc-coupon-search" style="width: 50%;" id="_coupon_title" name="_coupon_title" data-placeholder="<?php _e( 'Search for a coupon&hellip;', WC_SC_TEXT_DOMAIN ); ?>" data-action="sc_json_search_coupons" data-multiple="true" data-selected="<?php
				...
				?>" value="<?php echo implode( ',', array_keys( $json_coupons ) ); ?>" /> <?php
		    }
				echo wc_help_tip( __( 'These coupon/s will be given to customers who buy this product. The coupon code will be automatically sent to their email address on purchase.', WC_SC_TEXT_DOMAIN ) );
			?>
				</p>
				...
				<script type="text/javascript">
					...
					var updateSendCouponOnRenewals = function() {
						var prodType = jQuery('select#product-type').find('option:selected').val(); <?php

						if ( $this->is_wc_gte_30() ) { ?>
					        var associatedCouponCount = jQuery(' span.select2-selection ul.select2-selection__rendered li.select2-selection__choice').length; <?php
					    } else { ?>
						    var associatedCouponCount = jQuery('div[id^="s2id__coupon_title"] ul.select2-choices li.select2-search-choice').length; <?php
                        } ?>
			...
			woocommerce_wp_checkbox( array( 'id' => 'send_coupons_on_renewals', 'label' => __( 'Send coupons on renewals?', WC_SC_TEXT_DOMAIN ), 'description' => __( 'Check this box to send above coupons on each renewal order.', WC_SC_TEXT_DOMAIN ) ) );
		...
		public function woocommerce_process_product_meta_coupons( $post_id, $post ) {
			...
			if ( $_POST['product-type'] == 'subscription' || $_POST['product-type'] == 'variable-subscription' || $_POST['product-type'] == 'subscription_variation' ) {
