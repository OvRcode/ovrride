...
use YaySMTPAmazonSES\Aws3\Aws\Api\DocModel;
 ...
 */
class AwsClient implements \YaySMTPAmazonSES\Aws3\Aws\AwsClientInterface
{
    use AwsClientTrait;
    /** @var array */
    private $config;
    ...
    {
        return \YaySMTPAmazonSES\Aws3\Aws\ClientResolver::getDefaultArguments();
    }
    /**
    ...
    public function __construct(array $args)
    {
        ...
        $this->handlerList = new \YaySMTPAmazonSES\Aws3\Aws\HandlerList();
        $resolver = new \YaySMTPAmazonSES\Aws3\Aws\ClientResolver(static::getArguments());
        $config = $resolver->resolve($args, $this->handlerList);
        $this->api = $config['api'];
        $this->signatureProvider = $config['signature_provider'];
        $this->endpoint = new \YaySMTPAmazonSES\Aws3\GuzzleHttp\Psr7\Uri($config['endpoint']);
    ...
    public function getCommand($name, array $args = [])
    {
        ...
        return new \YaySMTPAmazonSES\Aws3\Aws\Command($name, $args, clone $this->getHandlerList());
    }
    ...
    private function parseClass()
    {
        ...
        if ($klass === __CLASS__) {
            return ['', 'YaySMTPAmazonSES\\Aws3\\Aws\\Exception\\AwsException'];
        }
        $service = substr($klass, strrpos($klass, '\\') + 1, -6);
        return [strtolower($service), "YaySMTPAmazonSES\\Aws3\\Aws\\{$service}\\Exception\\{$service}Exception"];
    }
    private function addSignatureMiddleware()
    {
        ...
        $resolver = static function (\YaySMTPAmazonSES\Aws3\Aws\CommandInterface $c) use($api, $provider, $name, $region, $version) {
            ...
            return \YaySMTPAmazonSES\Aws3\Aws\Signature\SignatureProvider::resolve($provider, $version, $name, $region);
        };
        $this->handlerList->appendSign(\YaySMTPAmazonSES\Aws3\Aws\Middleware::signer($this->credentialProvider, $resolver), 'signer');
    }
    private function addInvocationId()
    {
        // Add invocation id to each request
        $this->handlerList->prependSign(\YaySMTPAmazonSES\Aws3\Aws\Middleware::invocationId(), 'invocation-id');
    }
    /**
    ...
    {
        return [new \YaySMTPAmazonSES\Aws3\Aws\Api\Service($api, \YaySMTPAmazonSES\Aws3\Aws\Api\ApiProvider::defaultProvider()), new \YaySMTPAmazonSES\Aws3\Aws\Api\DocModel($docs)];
    }
    /**
     * @deprecated
     * @return static
