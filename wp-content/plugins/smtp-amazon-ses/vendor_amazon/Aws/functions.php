...
function default_http_handler() {
  $version = (string) \YaySMTPAmazonSES\Aws3\GuzzleHttp\ClientInterface::VERSION;
  if ($version[0] === '5') {
    return new \YaySMTPAmazonSES\Aws3\Aws\Handler\GuzzleV5\GuzzleHandler();
  }
  if ($version[0] === '6') {
    return new \YaySMTPAmazonSES\Aws3\Aws\Handler\GuzzleV6\GuzzleHandler();
  }
 ...
 */
function serialize(\YaySMTPAmazonSES\Aws3\Aws\CommandInterface $command) {
  $request = null;
  $handlerList = $command->getHandlerList();
  // Return a mock result.
  $handlerList->setHandler(function (\YaySMTPAmazonSES\Aws3\Aws\CommandInterface $_, \YaySMTPAmazonSES\Aws3\Psr\Http\Message\RequestInterface $r) use (&$request) {
    $request = $r;
    return new \YaySMTPAmazonSES\Aws3\GuzzleHttp\Promise\FulfilledPromise(new \YaySMTPAmazonSES\Aws3\Aws\Result([]));
  });
