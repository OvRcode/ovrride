...
namespace YaySMTPAmazonSES\Aws3\Aws\Multipart;

use YaySMTPAmazonSES\Aws3\Aws\AwsClientInterface as Client;
use YaySMTPAmazonSES\Aws3\Aws\CommandInterface;
use YaySMTPAmazonSES\Aws3\Aws\CommandPool;
use YaySMTPAmazonSES\Aws3\Aws\Exception\AwsException;
use YaySMTPAmazonSES\Aws3\Aws\Exception\MultipartUploadException;
 ...
 * @internal
 */
abstract class AbstractUploadManager implements \YaySMTPAmazonSES\Aws3\GuzzleHttp\Promise\PromisorInterface
{
    const DEFAULT_CONCURRENCY = 5;
    /** @var array Default values for base multipart configuration */
    private static $defaultConfig = ['part_size' => null, 'state' => null, 'concurrency' => self::DEFAULT_CONCURRENCY, 'prepare_data_source' => null, 'before_initiate' => null, 'before_upload' => null, 'before_complete' => null, 'exception_class' => 'YaySMTPAmazonSES\\Aws3\\Aws\\Exception\\MultipartUploadException'];
     ...
     * @param array  $config
     */
    public function __construct(\YaySMTPAmazonSES\Aws3\Aws\AwsClientInterface $client, array $config = [])
    {
        $this->client = $client;
    ...
    public function promise()
    {
        ...
        return $this->promise = \YaySMTPAmazonSES\Aws3\GuzzleHttp\Promise\coroutine(function () {
            // Initiate the upload.
            ...
            if (!$this->state->isInitiated()) {
                ...
                $this->state->setStatus(\YaySMTPAmazonSES\Aws3\Aws\Multipart\UploadState::INITIATED);
            }
            ...
            $commands = new \YaySMTPAmazonSES\Aws3\Aws\CommandPool($this->client, $this->getUploadCommands($resultHandler), ['concurrency' => $this->config['concurrency'], 'before' => $this->config['before_upload']]);
            ...
            $this->state->setStatus(\YaySMTPAmazonSES\Aws3\Aws\Multipart\UploadState::COMPLETED);
     ...
     */
    protected abstract function handleResult(\YaySMTPAmazonSES\Aws3\Aws\CommandInterface $command, \YaySMTPAmazonSES\Aws3\Aws\ResultInterface $result);
    /**
    ...
    private function determineState()
    {
        ...
        $state = new \YaySMTPAmazonSES\Aws3\Aws\Multipart\UploadState($id);
    ...
    protected function getResultHandler(&$errors = [])
    {
        return function (callable $handler) use(&$errors) {
            return function (\YaySMTPAmazonSES\Aws3\Aws\CommandInterface $command, \YaySMTPAmazonSES\Aws3\Psr\Http\Message\RequestInterface $request = null) use($handler, &$errors) {
                return $handler($command, $request)->then(function (\YaySMTPAmazonSES\Aws3\Aws\ResultInterface $result) use($command) {
                    $this->handleResult($command, $result);
                    return $result;
                }, function (\YaySMTPAmazonSES\Aws3\Aws\Exception\AwsException $e) use(&$errors) {
                    $errors[$e->getCommand()[$this->info['part_num']]] = $e;
                    return new \YaySMTPAmazonSES\Aws3\Aws\Result();
                });
            };
        };
    }
