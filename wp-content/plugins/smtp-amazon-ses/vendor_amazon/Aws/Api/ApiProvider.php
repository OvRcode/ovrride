...
use YaySMTPAmazonSES\Aws3\Aws\Exception\UnresolvedApiException;
/**
 * API providers.
 *
...
class ApiProvider
{
    ...
    public static function resolve(callable $provider, $type, $service, $version)
    {
        ...
        throw new \YaySMTPAmazonSES\Aws3\Aws\Exception\UnresolvedApiException($msg);
    }
    /**
    ...
    {
        return new self(__DIR__ . '/../data', \YaySMTPAmazonSES\Aws3\Aws\manifest());
    }
    /**
    ...
    public function __invoke($type, $service, $version)
    {
        ...
        try {
            return \YaySMTPAmazonSES\Aws3\Aws\load_compiled_json($path);
