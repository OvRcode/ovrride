...
class QueryParamBuilder {
  ...
  public function __invoke(\YaySMTPAmazonSES\Aws3\Aws\Api\StructureShape $shape, array $params) {
    if (!$this->methods) {
  ...
  protected function format(\YaySMTPAmazonSES\Aws3\Aws\Api\Shape $shape, $value, $prefix, array &$query) {
  ...
  protected function format_structure(\YaySMTPAmazonSES\Aws3\Aws\Api\StructureShape $shape, array $value, $prefix, &$query) {
    if ($prefix) {
      $prefix .= '.';
    }
  ...
  protected function format_map(\YaySMTPAmazonSES\Aws3\Aws\Api\MapShape $shape, array $value, $prefix, array &$query) {
    $vals = $shape->getValue();
    $keys = $shape->getKey();
    if (!$this->isFlat($shape)) {
      $prefix .= '.entry';
  ...
  protected function format_blob(\YaySMTPAmazonSES\Aws3\Aws\Api\Shape $shape, $value, $prefix, array &$query) {
    $query[$prefix] = base64_encode($value);
  }
  protected function format_timestamp(\YaySMTPAmazonSES\Aws3\Aws\Api\TimestampShape $shape, $value, $prefix, array &$query) {
    $query[$prefix] = \YaySMTPAmazonSES\Aws3\Aws\Api\TimestampShape::format($value, 'iso8601');
  }
  protected function format_boolean(\YaySMTPAmazonSES\Aws3\Aws\Api\Shape $shape, $value, $prefix, array &$query) {
