...
class TraceMiddleware {
  ...
  public function __invoke($step, $name) {
    ...
    return function (callable $next) use ($step, $name) {
      return function (\YaySMTPAmazonSES\Aws3\Aws\CommandInterface $command, \YaySMTPAmazonSES\Aws3\Psr\Http\Message\RequestInterface $request = null) use ($next, $step, $name) {
        ...
        return $next($command, $request)->then(function ($value) use ($step, $name, $command, $start) {
          ...
          return new \YaySMTPAmazonSES\Aws3\GuzzleHttp\Promise\RejectedPromise($reason);
        });
      };
    };
  }
  ...
  private function commandArray(\YaySMTPAmazonSES\Aws3\Aws\CommandInterface $cmd) {
    return ['instance' => spl_object_hash($cmd), 'name' => $cmd->getName(), 'params' => $cmd->toArray()];
  }
  private function requestArray(\YaySMTPAmazonSES\Aws3\Psr\Http\Message\RequestInterface $request = null) {
  ...
  private function responseArray(\YaySMTPAmazonSES\Aws3\Psr\Http\Message\ResponseInterface $response = null) {
  ...
  private function streamStr(\YaySMTPAmazonSES\Aws3\Psr\Http\Message\StreamInterface $body) {
    return $body->getSize() < $this->config['stream_size'] ? (string) $body : 'stream(size=' . $body->getSize() . ')';
  }
  private function createHttpDebug(\YaySMTPAmazonSES\Aws3\Aws\CommandInterface $command) {
    if ($this->config['http'] && !isset($command['@http']['debug'])) {
      $command['@http']['debug'] = fopen('php://temp', 'w+');
    }
  }
  private function flushHttpDebug(\YaySMTPAmazonSES\Aws3\Aws\CommandInterface $command) {
