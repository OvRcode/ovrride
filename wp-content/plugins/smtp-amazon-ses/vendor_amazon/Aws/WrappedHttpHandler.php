...
use YaySMTPAmazonSES\Aws3\Aws\Api\Parser\Exception\ParserException;
...
class WrappedHttpHandler {
   ...
   *                                 information.
   */
  public function __construct(callable $httpHandler, callable $parser, callable $errorParser, $exceptionClass = 'YaySMTPAmazonSES\\Aws3\\Aws\\Exception\\AwsException', $collectStats = false) {
   ...
   */
  public function __invoke(\YaySMTPAmazonSES\Aws3\Aws\CommandInterface $command, \YaySMTPAmazonSES\Aws3\Psr\Http\Message\RequestInterface $request) {
    $fn = $this->httpHandler;
    ...
    return \YaySMTPAmazonSES\Aws3\GuzzleHttp\Promise\promise_for($fn($request, $options))->then(function (\YaySMTPAmazonSES\Aws3\Psr\Http\Message\ResponseInterface $res) use ($command, $request, &$stats) {
      ...
      return new \YaySMTPAmazonSES\Aws3\GuzzleHttp\Promise\RejectedPromise($err);
    });
  }
  /**
   ...
   */
  private function parseResponse(\YaySMTPAmazonSES\Aws3\Aws\CommandInterface $command, \YaySMTPAmazonSES\Aws3\Psr\Http\Message\RequestInterface $request, \YaySMTPAmazonSES\Aws3\Psr\Http\Message\ResponseInterface $response, array $stats) {
    $parser = $this->parser;
    $status = $response->getStatusCode();
    $result = $status < 300 ? $parser($command, $response) : new \YaySMTPAmazonSES\Aws3\Aws\Result();
   ...
   * @return \Exception
   */
  private function parseError(array $err, \YaySMTPAmazonSES\Aws3\Psr\Http\Message\RequestInterface $request, \YaySMTPAmazonSES\Aws3\Aws\CommandInterface $command, array $stats) {
