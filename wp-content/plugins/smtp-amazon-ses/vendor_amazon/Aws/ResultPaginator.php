...
class ResultPaginator implements \Iterator {
   ...
   * @param array              $config
   */
  public function __construct(\YaySMTPAmazonSES\Aws3\Aws\AwsClientInterface $client, $operation, array $args, array $config) {
    $this->client = $client;
  ...
  public function each(callable $handleResult) {
    return \YaySMTPAmazonSES\Aws3\GuzzleHttp\Promise\coroutine(function () use ($handleResult) {
      $nextToken = null;
      do {
        ...
        if ($retVal !== null) {
          (yield \YaySMTPAmazonSES\Aws3\GuzzleHttp\Promise\promise_for($retVal));
        }
      } while ($nextToken);
    });
  }
  ...
  public function search($expression) {
    ...
    return flatmap($this, function (\YaySMTPAmazonSES\Aws3\Aws\Result $result) use ($expression) {
  ...
  private function determineNextToken(\YaySMTPAmazonSES\Aws3\Aws\Result $result) {
