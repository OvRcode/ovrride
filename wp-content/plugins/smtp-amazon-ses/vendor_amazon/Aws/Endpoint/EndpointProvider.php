...
namespace YaySMTPAmazonSES\Aws3\Aws\Endpoint;

use YaySMTPAmazonSES\Aws3\Aws\Exception\UnresolvedEndpointException;
/**
 * Endpoint providers.
 *
...
class EndpointProvider
{
    ...
    public static function resolve(callable $provider, array $args = [])
    {
        ...
        throw new \YaySMTPAmazonSES\Aws3\Aws\Exception\UnresolvedEndpointException('Unable to resolve an endpoint using the provider arguments: ' . json_encode($args) . '. Note: you can provide an "endpoint" ' . 'option to a client constructor to bypass invoking an endpoint ' . 'provider.');
    }
    /**
    ...
    {
        return \YaySMTPAmazonSES\Aws3\Aws\Endpoint\PartitionEndpointProvider::defaultProvider();
    }
    /**
    ...
    {
        return new \YaySMTPAmazonSES\Aws3\Aws\Endpoint\PatternEndpointProvider($patterns);
    }
}
