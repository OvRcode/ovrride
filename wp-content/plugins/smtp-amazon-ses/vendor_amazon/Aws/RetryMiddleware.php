...
class RetryMiddleware {
  ...
  public static function createDefaultDecider($maxRetries = 3) {
    ...
    return function ($retries, \YaySMTPAmazonSES\Aws3\Aws\CommandInterface $command, \YaySMTPAmazonSES\Aws3\Psr\Http\Message\RequestInterface $request, \YaySMTPAmazonSES\Aws3\Aws\ResultInterface $result = null, $error = null) use ($maxRetries, $retryCurlErrors) {
   ...
   */
  public function __invoke(\YaySMTPAmazonSES\Aws3\Aws\CommandInterface $command, \YaySMTPAmazonSES\Aws3\Psr\Http\Message\RequestInterface $request = null) {
    $retries = 0;
    $requestStats = [];
    ...
    $g = function ($value) use ($handler, $decider, $delay, $command, $request, &$retries, &$requestStats, &$g) {
      ...
      if ($value instanceof \Exception || $value instanceof \Throwable) {
        if (!$decider($retries, $command, $request, null, $value)) {
          return \YaySMTPAmazonSES\Aws3\GuzzleHttp\Promise\rejection_for($this->bindStatsToReturn($value, $requestStats));
        }
