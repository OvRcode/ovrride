 ...
 */
class EachPromise implements \YaySMTPAmazonSES\Aws3\GuzzleHttp\Promise\PromisorInterface
{
    private $pending = [];
    /** @var \Iterator */
    private $iterable;
    ...
    {
        $this->mutex = false;
        $this->aggregate = new \YaySMTPAmazonSES\Aws3\GuzzleHttp\Promise\Promise(function () {
            reset($this->pending);
            ...
            while ($promise = current($this->pending)) {
                next($this->pending);
                $promise->wait();
                if ($this->aggregate->getState() !== \YaySMTPAmazonSES\Aws3\GuzzleHttp\Promise\PromiseInterface::PENDING) {
                    return;
                }
            }
        });
