...
use YaySMTPAmazonSES\Aws3\GuzzleHttp\HandlerStack;
...
class MockHandler implements \Countable
{
    ...
    {
        return \YaySMTPAmazonSES\Aws3\GuzzleHttp\HandlerStack::create(new self($queue, $onFulfilled, $onRejected));
    }
    /**
    ...
    public function __invoke(\YaySMTPAmazonSES\Aws3\Psr\Http\Message\RequestInterface $request, array $options)
    {
        ...
        if (isset($options['on_headers'])) {
            ...
            try {
                ...
                $response = new \YaySMTPAmazonSES\Aws3\GuzzleHttp\Exception\RequestException($msg, $request, $response, $e);
            }
        }
        if (is_callable($response)) {
            $response = call_user_func($response, $request, $options);
        }
        $response = $response instanceof \Exception ? \YaySMTPAmazonSES\Aws3\GuzzleHttp\Promise\rejection_for($response) : \YaySMTPAmazonSES\Aws3\GuzzleHttp\Promise\promise_for($response);
    ...
    public function append()
    {
        foreach (func_get_args() as $value) {
            if ($value instanceof ResponseInterface || $value instanceof \Exception || $value instanceof PromiseInterface || is_callable($value)) {
                ...
                throw new \InvalidArgumentException('Expected a response or ' . 'exception. Found ' . \YaySMTPAmazonSES\Aws3\GuzzleHttp\describe_type($value));
            }
        }
    }
    /**
    ...
    private function invokeStats(\YaySMTPAmazonSES\Aws3\Psr\Http\Message\RequestInterface $request, array $options, \YaySMTPAmazonSES\Aws3\Psr\Http\Message\ResponseInterface $response = null, $reason = null)
    {
        if (isset($options['on_stats'])) {
            $stats = new \YaySMTPAmazonSES\Aws3\GuzzleHttp\TransferStats($request, $response, 0, $reason);
