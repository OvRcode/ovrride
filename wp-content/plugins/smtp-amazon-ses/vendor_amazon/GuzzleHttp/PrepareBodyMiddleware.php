    ...
    public function __invoke(\YaySMTPAmazonSES\Aws3\Psr\Http\Message\RequestInterface $request, array $options)
    {
        ...
        if (!$request->hasHeader('Content-Type')) {
            if ($uri = $request->getBody()->getMetadata('uri')) {
                if ($type = \YaySMTPAmazonSES\Aws3\GuzzleHttp\Psr7\mimetype_from_filename($uri)) {
        ...
        return $fn(\YaySMTPAmazonSES\Aws3\GuzzleHttp\Psr7\modify_request($request, $modify), $options);
    }
    private function addExpectHeader(\YaySMTPAmazonSES\Aws3\Psr\Http\Message\RequestInterface $request, array $options, array &$modify)
    {
