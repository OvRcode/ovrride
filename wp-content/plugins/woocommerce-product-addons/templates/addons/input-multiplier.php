...
<p class="form-row form-row-wide wc-pao-addon-wrap wc-pao-addon-<?php echo sanitize_title( $field_name ); ?>">
	<input type="number" class="input-text wc-pao-addon-field wc-pao-addon-input-multiplier" data-raw-price="<?php echo esc_attr( $price_raw ); ?>" data-price="<?php echo esc_attr( $price_display ); ?>" name="<?php echo $addon_key ?>" data-price-type="<?php echo esc_attr( $price_type ); ?>" value="" <?php if ( 1 === $restrictions || $min >= '0' ) echo 'min="' . esc_attr( $min ) .'"'; ?> <?php if ( ! empty( $max ) && 1 === $restrictions ) echo 'max="' . $max .'"'; ?>
