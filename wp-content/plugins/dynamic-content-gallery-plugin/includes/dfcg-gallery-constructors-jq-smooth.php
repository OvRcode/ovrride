...
function dfcg_jq_multioption_method_gallery() {
		...
		if( is_array($value) ) {
			...
			foreach ($value as $key => $value1) {
				...
				if( !$recent->have_posts() && $counter < 2 ) :
					...
					// Open the li element for each gallery item
	...
	$output .= "\n\t\t\t\t" . '<a id="dfcg-openGallery" class="dfcg-carouselBtn" title="Pictures">' . stripslashes($dfcg_options['textShowCarousel']) .'</a>';
...
*
* NOTE: This function was renamed in v3.3 (formally dfcg_jq_pages_method_gallery() )
*
* @uses dfcg_postmeta_info()		Builds array of postmeta key names (see dfcg-gallery-core.php)
* @uses	dfcg_errors_output()		Gets all Error Messages, if errors are on (see dfcg-gallery-errors.php)
* @uses	dfcg_baseimgurl()			Determines whether FULL or Partial URL applies (see dfcg-gallery-core.php)
...
* @since 3.3
*/
function dfcg_jq_id_method_gallery() {
