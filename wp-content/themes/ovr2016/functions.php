...
function wpb_modify_jquery() {
  ...
  if (!is_admin()) {
      ...
      wp_register_script('jquery', get_template_directory_uri() . '/includes/js/jquery.min.js', false, '2.2.4');
      // Enqueue the script   
...
function _tk_scripts() {
  ...
  #combined all commented out styles into master.min.css with cli tools.
  ...
  wp_enqueue_script( 'ovr_footer_script', THEME_DIR_URI . '/includes/js/ovr-footer.min.js', array('jquery'), "1.1.3", true);
  ...
  wp_enqueue_style( 'ovr_site_style', THEME_DIR_URI . '/includes/css/master.min.css', array(), "1.6.24");
}
...
function ovr_menu_mods($items, $args) {
  ...
  if ( is_user_logged_in() ) {
    ...
    $items .= '<li><a href="'. wp_logout_url() .'" title="Logout"><i class="fa fa-sign-out" aria-hidden="true"></a></i></li>';
  } else {
    $items .= '<li><a href="'. wp_login_url() .'" title="Sign In"><i class="fa fa-sign-in" aria-hidden="true"></a></i></li>';
  }
  return $items;
}
